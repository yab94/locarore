...
mep-ovh:
	docker compose exec php lftp ${FTP_LOGIN}:${FTP_PASSWORD}@${FTP_HOST}:/ -e "mirror -e -R -x docs* -x TODO* -x node_modules -x tests -x .env -x .git* -x sql* -x Makefile -x package* -x docker* -x README.md . /locarore ; quit"
