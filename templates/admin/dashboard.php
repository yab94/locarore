...
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
        ...
        <a href="/admin/reservations?status=pending" class="text-sm text-brand-600 hover:underline">Voir tout</a>
    </div>
        ...
        <tbody class="divide-y divide-gray-100">
            <?php foreach ($pendingList as $r): ?>
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-3"><?= e($r->getCustomerName()) ?></td>
                    <td class="px-6 py-3"><?= e($r->getStartDate()->format('d/m/Y')) ?> → <?= e($r->getEndDate()->format('d/m/Y')) ?></td>
                    <td class="px-6 py-3 text-gray-400"><?= e($r->getCreatedAt()->format('d/m/Y')) ?></td>
                    <td class="px-6 py-3">
                        <a href="/admin/reservations/<?= $r->getId() ?>" class="text-brand-600 hover:underline">Voir</a>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
