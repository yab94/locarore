...
$product    = Product::castOrNull($tpl->tryGet('product', null));
$categories = Cast::array($tpl->tryGet('categories', []));
$allTags    = Cast::array($tpl->tryGet('allTags', []));
        ...
        <form id="product-form" method="post"
            ...
            <div>
                ...
                <textarea name="description_short" rows="2"
                          ...
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-600 resize-y"><?= $html($product?->getDescriptionShort() ?? '') ?></textarea>
                <p class="text-xs text-gray-400 mt-1">Texte brut uniquement — affiché dans les cartes produit</p>
            </div>
